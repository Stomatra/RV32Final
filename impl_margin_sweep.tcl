...
set out_dir [file join $root_dir "timing_backups" "impl_margin_sweep_[clock format [clock seconds] -format %Y%m%d_%H%M%S]"]
file mkdir $out_dir

set candidates {
	{explore Explore Explore}
	{aggr_route Explore AggressiveExplore}
	{more_global Explore MoreGlobalIterations}
	{no_relax Explore NoTimingRelaxation}
}

set summary_path [file join $out_dir "summary.tsv"]
set summary_chan [open $summary_path w]
puts $summary_chan "name\tphys_directive\troute_directive\twns\treport\tdcp"
...
foreach candidate $candidates {
	lassign $candidate name phys_dir route_dir
	puts "===== IMPL SWEEP: $name / phys=$phys_dir / route=$route_dir ====="
	create_impl_design $name $synth_dcp $irom_xci $pll_xci $xdc_file
	opt_design
	place_design
	phys_opt_design -directive $phys_dir
	route_design -directive $route_dir -tns_cleanup
	...
	puts $summary_chan [format "%s\t%s\t%s\t%s\t%s\t%s" $name $phys_dir $route_dir $wns $rpt_path $dcp_path]
	flush $summary_chan
	close_project
}
