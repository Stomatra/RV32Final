if {[llength $argv] != 1} {
    puts "Usage: vivado -mode batch -source single_freq_explore_impl.tcl -tclargs <freq_mhz>"
    exit 1
}
...
set out_dir [file join $root_dir "timing_backups" "single_freq_explore_${freq_tag}_[clock format [clock seconds] -format %Y%m%d_%H%M%S]"]
file mkdir $out_dir
...
upgrade_ip $pll_ip
set_property -dict [list CONFIG.PRIMITIVE {PLL} CONFIG.CLKOUT1_REQUESTED_OUT_FREQ 50.0 CONFIG.CLKOUT2_REQUESTED_OUT_FREQ $target_freq] $pll_ip
...
create_project -in_memory probe_$freq_tag -part xc7k325tffg900-2
...
set rpt_path [file join $out_dir "timing_${freq_tag}.rpt"]
set dcp_path [file join $out_dir "top_${freq_tag}.dcp"]
report_timing_summary -max_paths 10 -report_unconstrained -file $rpt_path
write_checkpoint -force $dcp_path
puts "TARGET_FREQ_MHZ=$target_freq"
puts "FINAL_WNS=$wns"
puts "REPORT_PATH=$rpt_path"
