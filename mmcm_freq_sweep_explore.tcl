...
set target_freqs {200.0 200.5 201.0 201.5 202.0}
set out_dir [file join $root_dir "timing_backups" "mmcm_freq_sweep_explore_[clock format [clock seconds] -format %Y%m%d_%H%M%S]"]
file mkdir $out_dir
...
foreach freq $target_freqs {
    ...
    set status ok
    set wns NA
    set report_path [file join $out_dir "timing_mmcm_${tag}.rpt"]
    set dcp_path [file join $out_dir "top_mmcm_${tag}.dcp"]
    puts "===== MMCM FREQ SWEEP $freq MHz ====="
    ...
    if {[catch {
        ...
        set_property -dict [list \
            ...
            CONFIG.CLKOUT2_REQUESTED_OUT_FREQ $freq] $pll_ip
    ...
    if {$status eq "ok" && $wns ne "NA"} {
        if {$wns > 0 && ($best_freq eq "" || $freq > $best_freq)} {
            set best_freq $freq
            set best_wns $wns
...
# Restore baseline project clocking.
upgrade_ip $pll_ip
