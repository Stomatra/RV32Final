set project_dir "build_outputs/cpu_hdmi_status_project"
set project_name "cpu_hdmi_status"
set build_tag "CPU_HDMI_STATUS_720p60_200m_[clock format [clock seconds] -format {%Y%m%d_%H%M%S}]"
set bit_file "cpu_hdmi_status.bit"
...
set rtl_sv_files [list \
    digital_twin.srcs/sources_1/new/top_cpu_hdmi_status.sv \
...
add_files -norecurse -fileset constrs_1 digital_twin.srcs/constrs_1/new/cpu_hdmi_status_only.xdc
set_property top top_cpu_hdmi_status [current_fileset]
...
proc dump_drc_and_exit {stage} {
    set rpt "build_outputs/cpu_hdmi_status_drc_${stage}.rpt"
    catch {report_drc -file $rpt}
    puts "CPU_HDMI_STATUS_FAILED_STAGE=$stage"
    puts "CPU_HDMI_STATUS_DRC_REPORT=[file normalize $rpt]"
    if {[file exists $rpt]} {
        set fp [open $rpt r]
        puts [read $fp]
        close $fp
...
proc run_or_drc {stage cmd} {
    ...
    if {$code != 0} {
        puts "CPU_HDMI_STATUS_ERROR=$result"
        dump_drc_and_exit $stage
    }
}

run_or_drc synth {
    synth_design -top top_cpu_hdmi_status -part xc7k325tffg900-2
}
...
puts $fp "TOP=top_cpu_hdmi_status"
...
puts "CPU_HDMI_STATUS_BIT=[file normalize $bit_file]"
puts "CPU_HDMI_STATUS_FINAL_BIT=[file normalize $final_bit]"
puts "CPU_HDMI_STATUS_SUMMARY=[file normalize $summary]"
puts "CPU_HDMI_STATUS_DONE"
exit 0
