...
set project_dir "build_outputs/uart_rx_echo_${mode}_project"
set project_name "uart_rx_echo_${mode}"
set timestamp [clock format [clock seconds] -format {%Y%m%d_%H%M%S}]
if {$is_hdmi} {
    set top_module "top_cpu_hdmi_uart_rx_echo_status"
    set bit_file "cpu_hdmi_uart_rx_echo_status.bit"
    set build_tag "CPU_HDMI_UART_RX_ECHO_STATUS_720p60_200m_${timestamp}"
} else {
    set top_module "top_uart_rx_echo_test"
    set bit_file "uart_rx_echo_test.bit"
    set build_tag "UART_RX_ECHO_TEST_200m_${timestamp}"
}

set source_irom "digital_twin.srcs/sources_1/imports/test_src/irom-uart-echo.coe"
...
set generated_irom_sv "build_outputs/generated_irom_uart_echo.sv"
...
proc refresh_irom_ip_outputs {irom_xci} {
    ...
    set ip_project_dir "build_outputs/uart_rx_echo_ip_refresh_project"
    create_project uart_rx_echo_ip_refresh $ip_project_dir -part xc7k325tffg900-2 -force
...
if {![file exists $source_irom]} {
    error "Cannot find UART echo IROM: $source_irom. Run python scripts/gen_irom_uart_echo.py first."
}
...
set common_sv_files [list \
    build_outputs/generated_irom_uart_echo.sv \
...
set memory_files [list \
    ...
    digital_twin.srcs/sources_1/imports/test_src/irom-uart-echo.coe \
...
proc dump_drc_and_exit {stage} {
    set rpt "build_outputs/uart_rx_echo_drc_${stage}.rpt"
    catch {report_drc -file $rpt}
    puts "UART_RX_ECHO_FAILED_STAGE=$stage"
    puts "UART_RX_ECHO_DRC_REPORT=[file normalize $rpt]"
    if {[file exists $rpt]} {
        set fp [open $rpt r]
        puts [read $fp]
        close $fp
...
proc run_or_drc {stage cmd} {
    ...
    if {$code != 0} {
        puts "UART_RX_ECHO_ERROR=$result"
...
puts $fp "EXPECTED_SERIAL=typed characters echoed at 115200; CR/LF also appends CRLF"
puts $fp "EXPECTED_LED=starts 0x00000001, increments per received character"
puts $fp "EXPECTED_SEG=received character count"
...
puts "UART_RX_ECHO_MODE=$mode"
puts "UART_RX_ECHO_BIT=[file normalize $bit_file]"
puts "UART_RX_ECHO_FINAL_BIT=[file normalize $final_bit]"
puts "UART_RX_ECHO_BIT_SHA256=$bit_sha"
puts "UART_RX_ECHO_IROM_SHA256=$irom_sha"
puts "UART_RX_ECHO_IROM_FIRST8=$irom_first8"
puts "UART_RX_ECHO_WNS=$worst_setup"
puts "UART_RX_ECHO_WHS=$worst_hold"
puts "UART_RX_ECHO_DRC_ERROR_COUNT=$drc_error_count"
puts "UART_RX_ECHO_SUMMARY=[file normalize $summary]"
puts "UART_RX_ECHO_DONE"
exit 0
