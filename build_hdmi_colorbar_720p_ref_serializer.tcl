set project_dir "build_outputs/hdmi_colorbar_720p_ref_serializer_project"
set project_name "hdmi_colorbar_720p_ref_serializer"
set build_tag "HDMI_COLORBAR_720p60_ref_serializer_200m_[clock format [clock seconds] -format {%Y%m%d_%H%M%S}]"
set bit_file "hdmi_colorbar_720p_ref_serializer.bit"
...
set rtl_files [list \
    digital_twin.srcs/sources_1/new/hdmi_colorbar_720p_ref_serializer_top.sv \
    digital_twin.srcs/sources_1/new/hdmi_clock_gen_720p_ref.sv \
    digital_twin.srcs/sources_1/new/hdmi_demo_720p_ref.sv \
...
add_files -norecurse -fileset constrs_1 digital_twin.srcs/constrs_1/new/hdmi_colorbar_720p_ref_serializer.xdc
set_property top hdmi_colorbar_720p_ref_serializer_top [current_fileset]
update_compile_order -fileset sources_1

proc dump_drc_and_exit {stage} {
    set rpt "build_outputs/hdmi_colorbar_720p_ref_serializer_drc_${stage}.rpt"
    catch {report_drc -file $rpt}
    puts "HDMI_REF_SERIALIZER_FAILED_STAGE=$stage"
    puts "HDMI_REF_SERIALIZER_DRC_REPORT=[file normalize $rpt]"
    if {[file exists $rpt]} {
        set fp [open $rpt r]
        puts [read $fp]
        close $fp
...
proc run_or_drc {stage cmd} {
    ...
    if {$code != 0} {
        puts "HDMI_REF_SERIALIZER_ERROR=$result"
        dump_drc_and_exit $stage
    }
}

run_or_drc synth {
    synth_design -top hdmi_colorbar_720p_ref_serializer_top -part xc7k325tffg900-2
}
...
puts $fp "TOP=hdmi_colorbar_720p_ref_serializer_top"
puts $fp "VIDEO_MODE=1280x720@60"
puts $fp "INPUT_CLOCK_MODE=200m"
puts $fp "MMCM=CLKIN=200MHz DIVCLK=10 MULT=37.125 CLKOUT1_DIV=2"
puts $fp "PIXEL_CLK_MHZ=74.25"
puts $fp "SERIAL_CLK_MHZ=371.25"
puts $fp "SERIAL_CLOCK_BUFFER=BUFIO"
puts $fp "PIXEL_CLOCK_BUFFER=BUFR_DIVIDE_5_FROM_SERIAL_CLK"
puts $fp "SYSCLK_TO_HDMI_CMT_ROUTE=ANY_CMT_COLUMN"
puts $fp "HDMI_CLK_PATTERN=10'b0000011111"
...
puts "HDMI_REF_SERIALIZER_BIT=[file normalize $bit_file]"
puts "HDMI_REF_SERIALIZER_FINAL_BIT=[file normalize $final_bit]"
puts "HDMI_REF_SERIALIZER_SUMMARY=[file normalize $summary]"
puts "HDMI_REF_SERIALIZER_DONE"
exit 0
