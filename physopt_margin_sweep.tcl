...
set out_dir [file join $root_dir "timing_backups" "physopt_margin_sweep_[clock format [clock seconds] -format %Y%m%d_%H%M%S]"]
file mkdir $out_dir
...
puts $summary_chan "physopt_directive\twns\treport\tdcp\tstatus"
...
foreach directive $directives {
    ...
    if {[catch {
        create_impl_design $safe_name $synth_dcp $irom_xci $pll_xci $xdc_file
        opt_design
        place_design
