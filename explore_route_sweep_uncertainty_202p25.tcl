...
set out_dir [file join $root_dir "timing_backups" "explore_route_sweep_uncertainty_202p25_[clock format [clock seconds] -format %Y%m%d_%H%M%S]"]
file mkdir $out_dir
set delta [expr {5.000 - (1000.0 / 202.25)}]

proc create_impl_design {name synth_dcp irom_xci pll_xci xdc_file delta} {
...
create_impl_design explore_seed $synth_dcp $irom_xci $pll_xci $xdc_file $delta
opt_design
place_design -directive Explore
phys_opt_design
set explore_physopt_dcp [file join $out_dir "top_explore_physopt_202p25.dcp"]
...
foreach directive $directives {
    puts "===== 202P25 ROUTE SWEEP: $directive ====="
    set wns "NA"
    set status "ok"
