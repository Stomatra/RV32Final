...
set out_dir [file join $root_dir "timing_backups" "freq_probe_explore_linked_fixed_[clock format [clock seconds] -format %Y%m%d_%H%M%S]"]
file mkdir $out_dir
...
proc prepare_synth_for_freq {root_dir freq} {
    ...
    if {[llength $pll_ip] != 1 || [llength $pll_xci] != 1} {
        error "Could not resolve pll IP or pll_1 xci"
    }
...
proc create_impl_design {name root_dir} {
...
foreach freq $target_freqs {
    ...
    if {[catch {
        prepare_synth_for_freq $root_dir $freq
        create_impl_design probe_$tag $root_dir
        opt_design
