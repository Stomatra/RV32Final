...
set project_dir "build_outputs/uart_monitor_lite_${mode}_project"
set project_name "uart_monitor_lite_${mode}"
set timestamp [clock format [clock seconds] -format {%Y%m%d_%H%M%S}]
if {$is_hdmi} {
    set top_module "top_cpu_hdmi_uart_rx_echo_status"
    set bit_file "cpu_hdmi_uart_monitor_lite_status.bit"
    set build_tag "CPU_HDMI_UART_MONITOR_LITE_STATUS_720p60_200m_${timestamp}"
} else {
    set top_module "top_uart_rx_echo_test"
    set bit_file "uart_monitor_lite.bit"
    set build_tag "UART_MONITOR_LITE_200m_${timestamp}"
}

set source_irom "digital_twin.srcs/sources_1/imports/test_src/irom-uart-monitor-lite.coe"
...
set generated_irom_sv "build_outputs/generated_irom_uart_monitor_lite.sv"
...
proc refresh_irom_ip_outputs {irom_xci} {
    ...
    set ip_project_dir "build_outputs/uart_monitor_lite_ip_refresh_project"
    create_project uart_monitor_lite_ip_refresh $ip_project_dir -part xc7k325tffg900-2 -force
...
if {![file exists $source_irom]} {
    error "Cannot find UART monitor IROM: $source_irom. Run python scripts/gen_irom_uart_monitor_lite.py first."
}
...
puts "UART_MONITOR_LITE_MODE=$mode"
...
set common_sv_files [list \
    build_outputs/generated_irom_uart_monitor_lite.sv \
...
set memory_files [list \
    ...
    digital_twin.srcs/sources_1/imports/test_src/irom-uart-monitor-lite.coe \
...
proc dump_drc_and_exit {stage} {
    set rpt "build_outputs/uart_monitor_lite_drc_${stage}.rpt"
    catch {report_drc -file $rpt}
    puts "UART_MONITOR_LITE_FAILED_STAGE=$stage"
    puts "UART_MONITOR_LITE_DRC_REPORT=[file normalize $rpt]"
    if {[file exists $rpt]} {
        set fp [open $rpt r]
        puts [read $fp]
        close $fp
...
proc run_or_drc {stage cmd} {
    ...
    if {$code != 0} {
        puts "UART_MONITOR_LITE_ERROR=$result"
...
puts $fp "MONITOR_COMMANDS=? s l g r c t"
puts $fp "DRAM_TEST_RANGE=0x80100000 first 16 words"
...
puts "UART_MONITOR_LITE_MODE=$mode"
puts "UART_MONITOR_LITE_BIT=[file normalize $bit_file]"
puts "UART_MONITOR_LITE_FINAL_BIT=[file normalize $final_bit]"
puts "UART_MONITOR_LITE_BIT_SHA256=$bit_sha"
puts "UART_MONITOR_LITE_IROM_SHA256=$irom_sha"
puts "UART_MONITOR_LITE_IROM_FIRST8=$irom_first8"
puts "UART_MONITOR_LITE_WNS=$worst_setup"
puts "UART_MONITOR_LITE_WHS=$worst_hold"
puts "UART_MONITOR_LITE_DRC_ERROR_COUNT=$drc_error_count"
puts "UART_MONITOR_LITE_SUMMARY=[file normalize $summary]"
puts "UART_MONITOR_LITE_DONE"
exit 0
