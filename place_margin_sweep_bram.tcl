...
set out_dir [file join $root_dir "timing_backups" "place_margin_sweep_bram_[clock format [clock seconds] -format %Y%m%d_%H%M%S]"]
file mkdir $out_dir

set configs {
    {wl_block {place_design -subdirective Floorplan.WLDrivenBlockPlacement}}
    {wl_block_psip {place_design -subdirective Floorplan.WLDrivenBlockPlacement -psip_options {bram_register_opt fanout_opt}}}
    {wl_block_congestion {place_design -subdirective {Floorplan.WLDrivenBlockPlacement GPlace.ReduceCongestion.high DPlace.ReducePinDensity.high}}}
}
