from pathlib import Path

BASE = 0x80000000
RANGES = [
    (0x800007FC, 0x800008F8),
    (0x80000F20, 0x80001080),
]
PATH = Path(r"D:/digital_twin/digital_twin/digital_twin.srcs/sources_1/imports/test_src/irom.coe")

hexes = []
for line in PATH.read_text().splitlines()[2:]:
    s = line.strip().rstrip(',;')
    if len(s) == 8 and all(c in '0123456789abcdefABCDEF' for c in s):
        hexes.append(int(s, 16))

regs = ['x0','ra','sp','gp','tp','t0','t1','t2','s0','s1','a0','a1','a2','a3','a4','a5','a6','a7','s2','s3','s4','s5','s6','s7','s8','s9','s10','s11','t3','t4','t5','t6']

def sext(v, bits):
    return v - (1 << bits) if v >> (bits - 1) else v

def decode(ins, pc):
    op = ins & 0x7F
    rd = (ins >> 7) & 0x1F
    f3 = (ins >> 12) & 7
    rs1 = (ins >> 15) & 0x1F
    rs2 = (ins >> 20) & 0x1F
    f7 = (ins >> 25) & 0x7F
    if op == 0x03:
        imm = sext(ins >> 20, 12)
        names = {0: 'lb', 1: 'lh', 2: 'lw', 4: 'lbu', 5: 'lhu'}
        return f"{names.get(f3, 'load?')} {regs[rd]}, {imm}({regs[rs1]})"
    if op == 0x13:
        imm = sext(ins >> 20, 12)
        if f3 == 1:
            return f"slli {regs[rd]}, {regs[rs1]}, {(ins >> 20) & 0x1F}"
        if f3 == 5:
            kind = 'srai' if f7 == 0x20 else 'srli'
            return f"{kind} {regs[rd]}, {regs[rs1]}, {(ins >> 20) & 0x1F}"
        names = {0: 'addi', 2: 'slti', 3: 'sltiu', 4: 'xori', 6: 'ori', 7: 'andi'}
        return f"{names.get(f3, 'opimm?')} {regs[rd]}, {regs[rs1]}, {imm}"
    if op == 0x17:
        imm = ins & 0xFFFFF000
        return f"auipc {regs[rd]}, 0x{imm >> 12:x}"
    if op == 0x23:
        imm = sext(((ins >> 25) << 5) | ((ins >> 7) & 0x1F), 12)
        names = {0: 'sb', 1: 'sh', 2: 'sw'}
        return f"{names.get(f3, 'store?')} {regs[rs2]}, {imm}({regs[rs1]})"
    if op == 0x33:
        names = {0: 'sub', 5: 'sra'} if f7 == 0x20 else {0: 'add', 1: 'sll', 2: 'slt', 3: 'sltu', 4: 'xor', 5: 'srl', 6: 'or', 7: 'and'}
        return f"{names.get(f3, 'op?')} {regs[rd]}, {regs[rs1]}, {regs[rs2]}"
    if op == 0x37:
        imm = ins & 0xFFFFF000
        return f"lui {regs[rd]}, 0x{imm >> 12:x}"
    if op == 0x63:
        imm = (((ins >> 31) & 1) << 12) | (((ins >> 7) & 1) << 11) | (((ins >> 25) & 0x3F) << 5) | (((ins >> 8) & 0xF) << 1)
        imm = sext(imm, 13)
        names = {0: 'beq', 1: 'bne', 4: 'blt', 5: 'bge', 6: 'bltu', 7: 'bgeu'}
        return f"{names.get(f3, 'br?')} {regs[rs1]}, {regs[rs2]}, 0x{(pc + imm) & 0xFFFFFFFF:08x}"
    if op == 0x67:
        imm = sext(ins >> 20, 12)
        return f"jalr {regs[rd]}, {imm}({regs[rs1]})"
    if op == 0x6F:
        imm = (((ins >> 31) & 1) << 20) | (((ins >> 12) & 0xFF) << 12) | (((ins >> 20) & 1) << 11) | (((ins >> 21) & 0x3FF) << 1)
        imm = sext(imm, 21)
        return f"jal {regs[rd]}, 0x{(pc + imm) & 0xFFFFFFFF:08x}"
    return 'unknown'

for start, end in RANGES:
    print(f'=== 0x{start:08x}..0x{end:08x} ===')
    for pc in range(start, end + 1, 4):
        idx = (pc - BASE) // 4
        if idx >= len(hexes):
            break
        ins = hexes[idx]
        print(f"0x{pc:08x}: {ins:08x}  {decode(ins, pc)}")
    print()