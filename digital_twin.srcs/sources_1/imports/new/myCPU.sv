...
module myCPU #(
	...
	localparam logic [2:0]  WB_SRC_ZH     = 3'd6;// 写回数据选择 Z 多周期指令结果。
	...
	logic        id_z_heavy_hit;
	logic [3:0]  id_z_heavy_op;
	logic        id_z_heavy_uses_rs1;
	logic        id_z_heavy_uses_rs2;
	...
	logic        idex1_is_z_heavy;
	logic [3:0]  idex1_zh_op;
	...
	logic        ex1ex2_is_z_heavy;
	logic [3:0]  ex1ex2_zh_op;
	...
	logic        zh_busy;
	logic        zh_done;
	logic        zh_result_ready;
	logic        zh_stall;
	logic        zh_issue_hazard;
	logic [31:0] zh_result;
	...
	z_heavy_decode u_z_heavy_decode (
		.instr      (ifid_instr),
		.z_hit      (id_z_heavy_hit),
		.z_op       (id_z_heavy_op),
		.z_uses_rs1(id_z_heavy_uses_rs1),
		.z_uses_rs2(id_z_heavy_uses_rs2)
	);

	z_heavy_unit u_z_heavy_unit (
		.clk   (cpu_clk),
		.rst   (cpu_rst),
		.start (zh_start),
		.op    (ex1ex2_zh_op),
	...
	assign zh_start = ex1ex2_valid && ex1ex2_is_z_heavy && !zh_busy && !zh_done && !ex2_pc_redirect;
	assign zh_result_ready = zh_done;
	assign zh_stall = ex1ex2_valid && ex1ex2_is_z_heavy && !zh_result_ready;
	...
	assign fetch_stall = load_use_ex1_hazard || load_use_ex2_hazard ||
						 ...
						 m_issue_hazard || zh_issue_hazard || mem_load_stall || m_stall || zh_stall;
	...
	always_comb begin
		...
		if (ifid_valid) begin
			...
			end else if (id_z_heavy_hit) begin
				id_uses_rs1   = id_z_heavy_uses_rs1;
				id_uses_rs2   = id_z_heavy_uses_rs2;
				id_rf_we      = (id_rd != 5'd0);
				id_wb_sel     = WB_SRC_ZH;
				id_is_z_heavy = 1'b1;
				id_zh_op      = id_z_heavy_op;
			end else begin
				case (id_opcode)
				OPC_LUI: begin
					id_rf_we  = 1'b1;
	...
	assign zh_issue_hazard = idex1_valid && idex1_is_z_heavy;
	...
	always_ff @(posedge cpu_clk or posedge cpu_rst) begin
		if (cpu_rst) begin
		...
		end else if (mem_load_stall || m_stall || zh_stall) begin
			...
			idex1_is_z_heavy    <= id_is_z_heavy;
			idex1_zh_op         <= id_zh_op;
	...
	always_ff @(posedge cpu_clk or posedge cpu_rst) begin
		if (cpu_rst) begin
			...
			ex1ex2_is_z_heavy <= idex1_is_z_heavy;
			ex1ex2_zh_op <= idex1_zh_op;
		end
	end
	...
	always_comb begin
		...
		end else if (ex1ex2_is_z_heavy) begin
			ex2_wb_data = zh_result;
		end else begin
			case (ex1ex2_wb_sel)
	...
	always_ff @(posedge cpu_clk) begin
		...
		end else if (m_stall || zh_stall) begin
			ex2mem_valid      <= 1'b0;
			ex2mem_alu_y      <= 32'h0;
			ex2mem_store_data <= 32'h0;
			ex2mem_rd         <= 5'h0;
