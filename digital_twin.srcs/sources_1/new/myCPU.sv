...
module myCPU #(
	...
	assign ex_m_is_div = idex_valid && idex_is_m_ext &&
						 ((idex_m_op == M_OP_DIV) ||
						  ...
						  (idex_m_op == M_OP_REM) ||
						  (idex_m_op == M_OP_REMU));
	assign ex_m_is_mul = idex_valid && idex_is_m_ext && !ex_m_is_div && (idex_m_op != M_OP_NONE);
	...
	always_comb begin
		case (m_op_reg)
			...
			M_OP_MULH:   ex_mul_result_comb = m_mul_uu_reg[63:32]
										 - (m_rs1_reg[31] ? m_rs2_reg : 32'h0)
										 - (m_rs2_reg[31] ? m_rs1_reg : 32'h0);
			M_OP_MULHSU: ex_mul_result_comb = m_mul_uu_reg[63:32]
										 - (m_rs1_reg[31] ? m_rs2_reg : 32'h0);
			M_OP_MULHU:  ex_mul_result_comb = m_mul_uu_reg[63:32];
	...
	always_comb begin
		case (idex_m_op)
			...
			M_OP_MULH,
			M_OP_MULHSU,
			M_OP_MULHU:  ex_m_result = ex_m_result_reg;
			M_OP_DIV,
			M_OP_DIVU,
			M_OP_REM,
			M_OP_REMU:  ex_m_result = ex_m_result_reg;
